...
RepoGuard is built like a Claude Code/Codex-style developer agent, but it specializes in repository security review instead of general coding. A main orchestrator coordinates multiple scanner agents in parallel. Each sub-agent has a focused responsibility, such as filesystem risk analysis, Python security scanning, dependency vulnerability checks, secret scanning, license review, or multi-language SAST.
